...
class Elsie_Install_Plugins {
	...
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Main ) ) {
			self::$instance = new Elsie_Install_Plugins();
		}
	...
	public function do_plugin_install(){
		...
		$plugins_title     = array(
			esc_html__( 'Elsie Companion', 'elsie' ),
			esc_html__('Starter Sites', 'elsie')
		);
		...
		foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
			...
			$links_attrs[$key] = array(
				...
				'data-activating-label' => /* translators: %s: plugin name */ sprintf( __( 'Activating %s', 'elsie' ), $progress_text_and_title ),
				'data-installing-label' => /* translators: %s: plugin name */ sprintf( __( 'Installing %s', 'elsie' ), $progress_text_and_title ),
				'data-activate-label'   => /* translators: %s: plugin name */ sprintf( __( 'Activate %s'  , 'elsie' ), $progress_text_and_title ),
				'data-install-label'    => /* translators: %s: plugin name */ sprintf( __( 'Install %s'   , 'elsie' ), $progress_text_and_title ),
				...
				'data-wpnonce'          => wp_create_nonce( 'elsie-pi_setup_nonce' )
			);
			...
			if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Install %s', 'elsie' ), $progress_text_and_title );
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Activate %s', 'elsie' ), $progress_text_and_title );
			}
		}

	?>
		<?php if ( empty( $not_installed_or_activated_plugins_id ) ) : ?>
		<div class="elsie-message elsie-pi-notice-wrapper elsie-pi-notice-install-now" style="display:inline-block;">
			<a class="button" href="<?php echo esc_url( admin_url( 'themes.php?page=advanced-import.php' ) ); ?>"><?php echo esc_html__( 'Import a demo', 'elsie' ); ?></a>
		</div>		
		<?php else : ?>
		<div class="elsie-message elsie-pi-notice-wrapper elsie-pi-notice-install-now" style="display:inline-block;">
			<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<a class="button button-primary elsie-install-now elsie-pi-not-installed" data-info='<?php echo wp_json_encode( $links_attrs);?>' ><?php echo $links_attrs[0]['button_label']; ?></a>
		</div>
		<?php endif; ?>
	<?php
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'elsie-plugin-install', get_template_directory_uri() . '/inc/onboarding/plugin-install.js', array( 'jquery' ), '', true );

		wp_localize_script(
			'elsie-plugin-install',
			'elsiePluginInstall',
			array(
				'activating' => esc_html__( 'Activating ', 'elsie' ),
			)
		);
		...
		wp_enqueue_script( 'elsie-plugin-install' );

	}
}

$elsie_plugin_install = new Elsie_Install_Plugins();