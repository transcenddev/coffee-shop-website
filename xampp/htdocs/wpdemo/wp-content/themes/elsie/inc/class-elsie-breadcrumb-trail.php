...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Elsie_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Elsie_Breadcrumb_Trail.
 * @return void
 */
function elsie_breadcrumb_trail( $args = array() ) {
	if ( is_front_page() ) {
		return;
	}

	if ( class_exists( 'Woocommerce' ) && is_product() ) {
		echo '<div class="elsie-breadcrumb-trail"><div class="container">';
	...
	$display = get_theme_mod('elsie_breadcrumbs_display', array( 'is_home', 'is_single', 'is_page', 'is_archive', 'is_search' ));

	if ( is_home() && in_array( 'is_home', $display ) 
		|| is_single() && in_array( 'is_single', $display ) 
		|| is_page() && in_array( 'is_page', $display ) 
		|| ( is_archive() || is_tag() ) && in_array( 'is_archive', $display ) 
		|| is_search() && in_array( 'is_search', $display ) 
	) {
		$breadcrumb = apply_filters( 'elsie_breadcrumb_trail_object', null, $args );

		if ( ! is_object( $breadcrumb ) )
			$breadcrumb = new Elsie_Breadcrumb_Trail( $args );
	
		return $breadcrumb->trail();
	}

}
add_action( 'elsie_after_header', 'elsie_breadcrumb_trail', -99 );
...
class Elsie_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Elsie_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Elsie_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Elsie_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'elsie_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$item_position = 0;
		$separator 		= get_theme_mod( 'breadcrumbs_separator', 'icon-double-angle-right' );
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				if ( 1 === $item_position && 1 < $item_count ) {
					...
					$item = str_replace( '<span itemprop="item">', '<span itemprop="name">', $item );
				}
				...
				if ( $item_count === $item_position ) {
					$meta .= sprintf( '<meta itemprop="url" content="%s" />', get_the_permalink() );
				}

				// Build the list item.
				$breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s' . elsie_get_svg_icon( $separator, false ) . '</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta );
			}
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="elsie-breadcrumb-trail" itemprop="breadcrumb"><div class="container">%3$s%4$s%5$s</div></%1$s>',
		...
		$breadcrumb = apply_filters( 'elsie_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'elsie' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'elsie' ),
			'home'                => esc_html__( 'Home',                                  'elsie' ),
			'error_404'           => esc_html__( '404 Not Found',                         'elsie' ),
			'archives'            => esc_html__( 'Archives',                              'elsie' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'elsie' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'elsie' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'elsie' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'elsie' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'elsie' ),
		...
		$this->labels = apply_filters( 'elsie_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'elsie_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'elsie_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'elsie' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'elsie' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'elsie' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'elsie' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'elsie' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'elsie' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'elsie' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'elsie' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'elsie' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'elsie' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'elsie' ) ) ) );
